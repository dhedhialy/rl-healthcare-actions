...
def _get_state_dim() -> int:
    """Read state_dim from dataset instead of hardcoding."""
...
class DirectMethodOPE:
    """Direct Method: train a Q-function on behavior data, evaluate V(pi_e) = E_s[sum_a pi_e(a|s) Q(s,a)]."""
    ...
    def fit(self, ds: FlatDataset, epochs: int = 20, batch_size: int = 2048):
        ...
        for epoch in range(epochs):
            ...
            for start in range(0, n, batch_size):
                ...
                s = ds.states[idx].to(self.device)
                a = ds.actions[idx].to(self.device)
                r = ds.rewards[idx].to(self.device).unsqueeze(1)
                s2 = ds.next_states[idx].to(self.device)
                d = ds.dones[idx].to(self.device).unsqueeze(1)
...
def apply_safety_mask(pi: np.ndarray, test: pl.DataFrame, malignancy_hadm: set) -> np.ndarray:
    ...
    Implements S1-S7 safety constraints.
    """
    test_hadm = test["hadm_id"].to_numpy()

    # S1: No ESA (action 5) with active malignancy
    malign_mask = np.isin(test_hadm, list(malignancy_hadm))
    pi[malign_mask, 5] = 0.0

    # S3: No platelet transfusion (action 2) if Plt >= 50K
    if "platelets" in test.columns:
        plt_vals = test["platelets"].fill_null(999.0).to_numpy()
        plt_mask = plt_vals >= 50.0
        pi[plt_mask, 2] = 0.0

    # S4: No FFP (action 3) if INR <= 2.0
    if "inr" in test.columns:
        inr_vals = test["inr"].fill_null(0.0).to_numpy()
        inr_mask = inr_vals <= 2.0
        pi[inr_mask, 3] = 0.0

    # S5: No vasopressor (action 9) if MAP >= 65 mmHg
    if "mean_bp" in test.columns:
        map_vals = test["mean_bp"].fill_null(100.0).to_numpy()
        map_mask = map_vals >= 65.0
        pi[map_mask, 9] = 0.0

    # S6: No insulin (action 11) if glucose < 70 mg/dL
    if "glucose" in test.columns:
        glu_vals = test["glucose"].fill_null(100.0).to_numpy()
        glu_mask = glu_vals < 70.0
        # ponytail: mask insulin only when glucose is actually measured low, fill_null(100) defaults to safe
        pi[glu_mask, 11] = 0.0

    # S7: No diuretic (action 12) if creatinine > 4.0 + hypotension (MAP < 65)
    if "creatinine" in test.columns and "mean_bp" in test.columns:
        ...
        s7_mask = (cr_vals > 4.0) & (map_vals2 < 65.0)
        pi[s7_mask, 12] = 0.0
...
def run_ope_suite(device: Optional[str] = None) -> Dict:
    ...
    results = {}
    for name, policy in [("iql", iql), ("bc", bc)]:
...
def _load_malignancy_hadm() -> set:
    malignancy_icd10_prefixes = [p for p in SAFETY_CONSTRAINTS[0]["icd_prefix"]]
...
def safety_audit(device: Optional[str] = None) -> Dict:
    """Check S1-S7 safety constraints after applying post-hoc action masking."""
    device = device or auto_device()
    dev = torch.device(device)
    iql = _load_best_iql(device)
    test_ds = FlatDataset(str(DS_DIR / "test.parquet"))
    ...
    # S1: No ESA with active malignancy
    ...
    # S3: Platelet transfusion only if Plt < 50K
    if "platelets" in test.columns:
        plt_vals = test["platelets"].fill_null(999.0).to_numpy()
        for idx in range(len(test)):
    ...
    # S7: No diuretic if creatinine > 4 + hypotension
    if "creatinine" in test.columns and "mean_bp" in test.columns:
        cr_vals = test["creatinine"].fill_null(0.0).to_numpy()
        map_vals2 = test["mean_bp"].fill_null(100.0).to_numpy()
        for idx in range(len(test)):
    ...
    # Pre-mask violation counts
    raw_actions = pi_raw.argmax(axis=1)
    pre_mask = {}
    pre_mask["S1"] = int(np.sum((raw_actions == 5) & np.isin(test_hadm, list(malignancy_hadm))))
    if "platelets" in test.columns:
        pre_mask["S3"] = int(np.sum((raw_actions == 2) & (plt_vals >= 50.0)))
    if "inr" in test.columns:
        pre_mask["S4"] = int(np.sum((raw_actions == 3) & (inr_vals <= 2.0)))
    if "mean_bp" in test.columns:
        pre_mask["S5"] = int(np.sum((raw_actions == 9) & (map_vals >= 65.0)))
    if "glucose" in test.columns:
        pre_mask["S6"] = int(np.sum((raw_actions == 11) & (glu_vals < 70.0)))
    if "creatinine" in test.columns and "mean_bp" in test.columns:
        pre_mask["S7"] = int(np.sum((raw_actions == 12) & (cr_vals > 4.0) & (map_vals2 < 65.0)))

    return {
        "n_violations": len(violations),
        "violations": violations[:20],
        "constraints_checked": [c["id"] for c in SAFETY_CONSTRAINTS],
        "pre_mask_violations": pre_mask,
        "post_mask_violations": len(violations),
    }


def phenotype_stratification(device: Optional[str] = None) -> Dict:
    """Evaluate per-phenotype-cluster policy performance using primary diagnosis grouping."""
    device = device or auto_device()
    dev = torch.device(device)
    iql = _load_best_iql(device)
    test_ds = FlatDataset(str(DS_DIR / "test.parquet"))
    ...
    test_with_diag = test_with_diag.with_columns(
        pl.when(pl.col("icd_version") == 10)
        .then(pl.col("icd_code").cast(pl.Utf8).str.slice(0, 3))
        .otherwise(pl.col("icd_code").cast(pl.Utf8).str.slice(0, 3))
        .alias("icd_group")
    )
...
def plausibility_check(device: Optional[str] = None, n_trajectories: int = 5) -> Dict:
    """Spot-check high-reward trajectories for clinical plausibility."""
    device = device or auto_device()
    dev = torch.device(device)
    iql = _load_best_iql(device)
    test_ds = FlatDataset(str(DS_DIR / "test.parquet"))
